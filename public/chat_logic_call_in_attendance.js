async function callInAttendance() {
    ...
    if (current_school.call_in_time && current_school.call_in_time.trim() !== "") {
        chatWindow.talk(callingInAttendanceLogic({school: current_school.name, time: friendlyTime(current_school.call_in_time)}), "already_called_in");
    } else {
...
function callingInAttendanceLogic({school, time}){
    return {
        ...
        "already_called_in": {
            says: [
                `You already called in today at ${time}.`,
            ],
            reply: [
                {question: "Start over", answer: "start_over"},
                {question: "Log out", answer: "log_out"},
